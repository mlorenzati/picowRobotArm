...
# MediaPipe 1.0 uses the Tasks API. The old mp.solutions API is no longer
# available, so the controller uses the native HolisticLandmarker while
# keeping the landmark access pattern used by the original controller.
...
MODEL_PATH = Path(__file__).resolve().parent / "holistic_landmarker.task"
...
class _PoseLandmark(IntEnum):
    """Pose landmark indices used by the original Solutions API."""
    LEFT_SHOULDER = 11
    RIGHT_SHOULDER = 12
    RIGHT_ELBOW = 14
    RIGHT_WRIST = 16
...
class _HandLandmark(IntEnum):
    """Hand landmark indices used by the original Solutions API."""
    WRIST = 0
    THUMB_CMC = 1
    INDEX_FINGER_MCP = 5
    INDEX_FINGER_PIP = 6
...
# Compatibility namespaces matching the names used by the original controller.
mp_pose = type("Pose", (), {"PoseLandmark": _PoseLandmark})
mp_hand = type("Hand", (), {"HandLandmark": _HandLandmark})

# Gripper servo range
GRIPPER_OPEN   = 100   # servo degrees when hand is fully open
GRIPPER_CLOSED = 180   # servo degrees when hand is fully closed


class _LandmarkContainer:
    """Compatibility wrapper matching the old solution API's .landmark field."""
...
def _as_landmark_list(value):
    """Normalize Tasks API output across flat and batched landmark layouts."""
    if not value:
        return None
    # HolisticLandmarker versions have returned either:
    #   [NormalizedLandmark, ...]
    # or [ [NormalizedLandmark, ...] ]
    first = value[0]
...
class _HolisticResultAdapter:
    """Adapt MediaPipe Tasks results to the fields used by this controller."""
...
# Tasks API connection definitions. The old controller only draws pose and
# right-hand landmarks, so only those two sets are needed here.
POSE_CONNECTIONS = mp_tasks.PoseLandmarksConnections.POSE_LANDMARKS

HAND_CONNECTIONS = mp_tasks.HandLandmarksConnections.HAND_CONNECTIONS
...
def calculate_y_up_matrix(v):
   # Normalize the vector
    ...
    rotation_angle = -np.arccos(np.dot(v, np.array([0.0, 1.0, 0.0])))
    ...
    rotation_matrix = (
        np.eye(3) +
        np.sin(rotation_angle) * axis_crossproduct_matrix +
        (1 - np.cos(rotation_angle)) * np.dot(axis_crossproduct_matrix, axis_crossproduct_matrix)
    )
...
def calculate_gripper(joint_xyz):
    """Calculate gripper open/close value from hand landmarks.

    Uses the dot-product voting approach from app2:
      For each of index, middle, ring: compute direction wrist→MCP (the
      "open/forward" reference) and direction MCP→PIP (finger extends away
      from palm when open).  If dot(wrist_dir, finger_dir) >= DOT_THRESHOLD
      the finger is considered extended (open).  2+ votes → OPEN.

    Returns (servo_value, is_open_bool, openness_0_to_1)
    """
    DOT_THRESHOLD = 0.7   # same threshold as app2

    # Reference: wrist → index-MCP  (points "forward" when hand is open)
    wrist_pt  = joint_xyz[mp_hand.HandLandmark.WRIST]
    index_mcp = joint_xyz[mp_hand.HandLandmark.INDEX_FINGER_MCP]

    wrist_dir = index_mcp - wrist_pt
    wrist_dir_norm = np.linalg.norm(wrist_dir)
    if wrist_dir_norm < 1e-8:
        return float(GRIPPER_OPEN), True, 1.0
    wrist_dir = wrist_dir / wrist_dir_norm

    # For each finger: direction from MCP toward PIP
    # A straight/extended finger has MCP→PIP aligned with wrist→index-MCP
    finger_pairs = [
        (mp_hand.HandLandmark.INDEX_FINGER_MCP,  mp_hand.HandLandmark.INDEX_FINGER_PIP),
        (mp_hand.HandLandmark.MIDDLE_FINGER_MCP, mp_hand.HandLandmark.MIDDLE_FINGER_PIP),
        (mp_hand.HandLandmark.RING_FINGER_MCP,   mp_hand.HandLandmark.RING_FINGER_PIP),
    ]

    votes = 0
    for mcp_idx, pip_idx in finger_pairs:
        mcp_pt = joint_xyz[mcp_idx]
        pip_pt = joint_xyz[pip_idx]
        finger_dir = pip_pt - mcp_pt
        fn = np.linalg.norm(finger_dir)
        if fn < 1e-8:
            continue
        finger_dir = finger_dir / fn
        # Extended finger: MCP→PIP points in same direction as wrist→index-MCP
        if np.dot(wrist_dir, finger_dir) >= DOT_THRESHOLD:
            votes += 1

    is_open   = votes >= 2
    openness  = float(votes) / 3.0          # 0.0 = closed, 0.33/0.67/1.0 = open
    servo_val = GRIPPER_OPEN if is_open else GRIPPER_CLOSED

    return float(servo_val), is_open, openness


def calculate_wrist_pitch(hand_points, pose_world_landmarks):
    """Calculate wrist pitch using the hand normal and forearm direction.

    Wrist pitch is the angle between the forearm direction (elbow → wrist)
    and the hand-forward direction (wrist → middle-MCP), measured around the
    palm-width axis (index-MCP → pinky-MCP).

    Returns pitch in degrees. Neutral (forearm and hand aligned) ≈ 90°.
    """
    right_elbow = landmark_to_np(pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW])
    right_wrist = landmark_to_np(pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST])

    # forearm direction: elbow → wrist
    forearm_dir = right_wrist - right_elbow
    norm_fa = np.linalg.norm(forearm_dir)
    if norm_fa < 1e-8:
        return 90.0
    forearm_dir = forearm_dir / norm_fa

    # hand-forward: wrist → middle-MCP (in pose world space, use hand_points
    # which have already been translated to pose frame)
    h_wrist  = hand_points[mp_hand.HandLandmark.WRIST]
    h_middle = hand_points[mp_hand.HandLandmark.MIDDLE_FINGER_MCP]
    h_index  = hand_points[mp_hand.HandLandmark.INDEX_FINGER_MCP]
    h_pinky  = hand_points[mp_hand.HandLandmark.PINKY_MCP]

    hand_fwd = h_middle - h_wrist
    norm_hf  = np.linalg.norm(hand_fwd)
    if norm_hf < 1e-8:
        return 90.0
    hand_fwd = hand_fwd / norm_hf

    # palm-width axis: index-MCP → pinky-MCP
    palm_width = h_pinky - h_index
    norm_pw    = np.linalg.norm(palm_width)
    if norm_pw < 1e-8:
        return 90.0
    palm_width = palm_width / norm_pw

    # Signed angle from forearm_dir to hand_fwd around palm_width axis
    cos_val  = np.clip(np.dot(forearm_dir, hand_fwd), -1.0, 1.0)
    cross    = np.cross(forearm_dir, hand_fwd)
    sin_val  = np.dot(cross, palm_width)
    pitch_delta = np.degrees(np.arctan2(sin_val, cos_val))

    # 90° = neutral (forearm and hand aligned)
    pitch = float(np.clip(90.0 + pitch_delta, 0.0, 180.0))
    return pitch
...
gripper_servo  = float(GRIPPER_OPEN)   # current servo value (smoothed)
gripper_is_open = True

# MediaPipe 1.0 HolisticLandmarker in VIDEO mode preserves tracking between
# frames and is the closest Tasks-API equivalent to the old Holistic solution.
ensure_holistic_model()
base_options = mp.tasks.BaseOptions(model_asset_path=str(MODEL_PATH))
holistic_options = mp_holistic.HolisticLandmarkerOptions(
...
with mp_holistic.HolisticLandmarker.create_from_options(holistic_options) as holistic:
  ...
  while cvcam.is_opened():
    ...
    # MediaPipe Tasks requires a MediaPipe Image and a monotonically increasing
    # timestamp when VIDEO mode is used.
    mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=image)
    now_ms = int(time.monotonic() * 1000)
    video_timestamp_ms = max(now_ms, last_timestamp_ms + 1)
    last_timestamp_ms = video_timestamp_ms
    raw_results = holistic.detect_for_video(mp_image, video_timestamp_ms)
    results = _HolisticResultAdapter(raw_results)

    # Draw the pose annotation on the image. The Tasks drawing utility uses
    # connection tuples instead of the old Solutions connection constants.
    ...
    hand_points = None
    wrist_rotation = 0.0
    wrist_pitch_angle = 90.0    # neutral default
    hand_points_pose_frame = None
    openness = 1.0
    ...
    if results.right_hand_landmarks is not None:
      ...
      zaxis_pt = hand_points_norm[mp_hand.HandLandmark.PINKY_MCP] + np.array([0.0,0.0,1.0]) 
      rel = index - pinky
      ...
      wrist_rotation = 180-angle(
        ...
        np.array([zaxis_pt[0], zaxis_pt[2]]))
      ...
      # Gripper open/close
      # ----------------------------------------------------------------
      gripper_raw, is_open_new, openness = calculate_gripper(hand_points_norm)

      # Smooth the gripper value the same way as other joint angles
      gripper_servo  = (1.0 - args.lpf_value) * gripper_servo + args.lpf_value * gripper_raw
      gripper_is_open = is_open_new

      # Store in joint_angles payload slot [17]
      joint_angles[17] = gripper_servo

      # ----------------------------------------------------------------
      # Translate hand to pose world frame for pitch calculation
      # ----------------------------------------------------------------
      if results.pose_world_landmarks is not None:
          pose_wrist = landmark_to_np(results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST])
          hand_points_pose_frame = deepcopy(hand_points)
          delta = pose_wrist - hand_points_pose_frame[0]
          hand_points_pose_frame += delta

          # Estimate the center of the palm
          hcp = (hand_points_pose_frame[0]+hand_points_pose_frame[5]+hand_points_pose_frame[17])/3.0

          # Compute the normal to the center of the palm
          hup = hand_points_pose_frame[9]-hand_points_pose_frame[0]
          hup /= np.linalg.norm(hup)

          hright = hand_points_pose_frame[5]-hand_points_pose_frame[17]
          ...
          # Wrist pitch
          wrist_pitch_angle = calculate_wrist_pitch(hand_points_pose_frame, results.pose_world_landmarks)
          joint_angles[16] = wrist_pitch_angle

          # Wrist roll (unchanged from original)
          joint_angles[18] = wrist_rotation
      else:
          hcp  = np.zeros(3)
          hncp = np.zeros((3, 3))
          hand_points_pose_frame = hand_points.copy()
    ...
    if is_valid_frame and show_debug_views:
        ...
        # Elbow
        ex = int(image.shape[1] - screen_right_elbow.x * image.shape[1])
        ey = int(screen_right_elbow.y * image.shape[0])
        cv2.rectangle(flipped_image, (ex+5, ey-15), (ex+110, ey+5), (0, 0, 0), -1)
        cv2.putText(flipped_image, "Elb: {:.1f}".format(right_elbow_angle), (ex+5, ey), cv2.FONT_HERSHEY_SIMPLEX, 0.5, visibilityToColour(screen_right_elbow.visibility), 1, cv2.LINE_AA)

        # Wrist — shows roll and pitch
        wx = int(image.shape[1] - screen_right_wrist.x * image.shape[1])
        wy = int(screen_right_wrist.y * image.shape[0])
        cv2.rectangle(flipped_image, (wx+5, wy-30), (wx+180, wy+5), (0, 0, 0), -1)
        cv2.putText(flipped_image, "Roll: {:.1f}".format(wrist_rotation), (wx+5, wy-15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, visibilityToColour(screen_right_wrist.visibility), 1, cv2.LINE_AA)
        cv2.putText(flipped_image, "Pitch: {:.1f}".format(wrist_pitch_angle), (wx+5, wy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, visibilityToColour(screen_right_wrist.visibility), 1, cv2.LINE_AA)
        
        # Shoulder
        sx = int(image.shape[1] - screen_right_shoulder.x * image.shape[1])
        sy = int(screen_right_shoulder.y * image.shape[0])
        cv2.rectangle(flipped_image, (sx+5, sy-15), (sx+220, sy+5), (0, 0, 0), -1)
        cv2.putText(flipped_image, "Sh Yaw:{:.1f} Pit:{:.1f}".format(right_shoulder_yaw, right_shoulder_pitch), (sx+5, sy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, visibilityToColour(screen_right_shoulder.visibility), 1, cv2.LINE_AA)

        # Gripper — shown in the top-left corner
        gripper_label = "OPEN" if gripper_is_open else "CLOSED"
        gripper_color = (0, 255, 120) if gripper_is_open else (0, 80, 255)
        cv2.rectangle(flipped_image, (5, 45), (280, 95), (0, 0, 0), -1)
        cv2.putText(flipped_image, "Gripper: {} ({:.1f})".format(gripper_label, gripper_servo),
                    (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, gripper_color, 2, cv2.LINE_AA)
        # Draw a simple openness bar
        bar_w = int(openness * 260)
        cv2.rectangle(flipped_image, (10, 75), (10+bar_w, 88), gripper_color, -1)
        cv2.rectangle(flipped_image, (10, 75), (270, 88), (128,128,128), 1)

    
    
    # Debug output
    if (is_valid_frame and show_debug_views and hand_points_pose_frame is not None):
      drawDebugViews(results, hand_points_pose_frame, hcp, hncp, hand_points_norm, pitchmode)
